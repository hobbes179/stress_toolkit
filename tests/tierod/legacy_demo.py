"""
tests/tierod/legacy_demo.py — the two-tank geometry the optimizer tests were
written against.

It used to be `apps.tierod.examples.demo_assembly`. It lives here now because
the shipped examples are a SHOWCASE and get replaced when a better
demonstration comes along — which happened on 2026-08-24, and took 69 tests
with it, all of them keyed to region names like `band_a` that no longer
existed.

A test fixture and a demo have different jobs. Tests need geometry that is
frozen so their numbers stay meaningful; demos need geometry that shows the
tool off. Keeping them the same object couples the two, and the demo always
wins. So this is a copy, deliberately, and it does not track the examples.
"""
...
#: The two cross-body region pairs, for `LayoutSpace.restrict`.
PAIRWISE = [("band_a", "foot_a"), ("band_b", "foot_b")]


def two_tank_demo() -> Assembly:
